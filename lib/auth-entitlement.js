 ...
 * If Supabase has any `users` row for this email, treat as "existing" so the modal
 * does not show "no account found" for real customers whose row is not yet
 * ghl_contact_id + subscription (or when CRM tags lag). POST /api/auth/login still
 * enforces real entitlement for tokens.
 */
